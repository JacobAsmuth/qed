// Real-browser test of the WASM-free, transpiled-from-Lean counter.
...
try {
  ...
  const clickBtn = (label) => page.evaluate((lab) => {
    [...document.querySelectorAll('#app button')].find((x) => x.textContent.trim() === lab).click();
  }, label);
  ...
  check('input keeps value (diff patched only the count)', await page.$eval('#app input', (e) => e.value), 'hello');
...
console.log(failures ? `\n${failures} FAILURES` : '\nReal-browser counter: ALL PASS (transpiled Lean, no WASM)');
