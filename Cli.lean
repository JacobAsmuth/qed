...
def linkWasm (outDir : FilePath) (prod : Bool) : IO Bool := do
  ...
  let altMains := ["Native.c", "Cli.c", "Web.c", "ChatWeb.c", "SignupWeb.c", "BookingWeb.c", "TodoWeb.c", "UsersWeb.c", "LocalWeb.c", "EffectsWeb.c", "Bench.c", "BenchAppWeb.c", "SignalsWeb.c"].filter (· ≠ entryC)
