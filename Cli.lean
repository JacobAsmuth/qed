/-
  ...
  Commands mirror the npm/vite vocabulary:
      ...
      qed dev      watch sources, rebuild, serve (SSR) with live-reload
      qed build    production build → dist/ (optimized + verified; client bundle + ssr.mjs)
      qed start    serve the production build, server-rendered  (alias: preview)
  ...
  `build`/`check` run verification: `lake build` (so the kernel checks
  the compiled proofs, a failed proof is a build error), a grep for
  `sorry`/`admit`/`native_decide`, and the axiom manifest if one is present.
  `dev` builds the selected entry and its dependencies, checking their proofs.
...
/-- The app value the web entry runs, read from the module's source: the `<app>` of
    `def main : IO Unit := Qed.run <app>`. This is what lets `qed build` emit the SSR
    handler with zero per-app code: the app value itself is exported to JS next to the
    generic `Qed.App.ssrStep`. `none` (entry not found or not in the `Qed.run app`
    shape) just skips the SSR bundle. -/
def ssrAppName (mod : String) : IO (Option String) := do
  let path := FilePath.mk ((mod.replace "." "/") ++ ".lean")
  if !(← path.pathExists) then return none
  let src ← IO.FS.readFile path
  match src.splitOn "Qed.run " with
  | _ :: rest :: _ =>
      let name := ((rest.dropWhile (· == ' ')).takeWhile fun c =>
        c.isAlphanum || c == '.' || c == '_' || c == '\'').toString
      return if name.isEmpty then none else some name
  | _ => return none

/-- The generated `ssr.mjs`: the app's request handler, importable from any node/edge
    server (`export default`), or served locally by `qed dev`/`qed start`. -/
def ssrWrapper (appName : String) (cacheKey : String := "") : String :=
  let title := (appName.splitOn ".").headD "qed"
  let appImport := "./ssr_app.mjs" ++ (if cacheKey.isEmpty then "" else s!"?build={cacheKey}")
  "// generated by qed build: the SSR request handler for `" ++ appName ++ "`.\n" ++
  "// deploy: serve the static files in this directory, and route every other GET\n" ++
  "// through this handler (any node/edge runtime; `qed start` does it locally).\n" ++
  "import * as mod from '" ++ appImport ++ "';\n" ++
  "import * as rt from './qed_rt.mjs';\n" ++
  "import { makeHandler } from './qed_ssr_host.mjs';\n" ++
  "rt.assertExterns(mod.__externs, rt);\n" ++
  "export { makeHandler, mod };   // build a custom handler: makeHandler(mod, { title, fetch, … })\n" ++
  "export default makeHandler(mod, { title: '" ++ title ++ "', script: '/qed_host.mjs' });\n"
...
def buildJs (outDir : FilePath) (prod : Bool) : IO Bool := do
  ...
  -- Let Lake resolve the executable, including when Qed is an app dependency.
  if (← sh "lake" (#["exe", "qedjs"] ++ minArgs ++ #[appOut, mod, "--"] ++ driverEntries)) != 0 then
    return false
  ...
  -- SSR: a second, driver-free transpile exporting the app value + the generic request
  -- step, wrapped as `ssr.mjs` (a standard request handler). Skipped, with a note, when
  -- the web entry isn't the `Qed.run <app>` shape.
  match ← ssrAppName mod with
  | some appName =>
      let ssrOut := (outDir / "ssr_app.mjs").toString
      if (← sh "lake" (#["exe", "qedjs"] ++ minArgs ++ #[ssrOut, mod, "--"] ++
            #[s!"{appName}:app", "Qed.App.ssrStep:ssrStep"])) != 0 then return false
      IO.FS.writeFile (outDir / "qed_ssr_host.mjs") (← IO.FS.readFile (← runtimeFile "qed_ssr_host.mjs"))
      -- The dev server reloads this wrapper; its app import must also escape Node's
      -- module cache, or server-rendered markup stays at the first build.
      let cacheKey ← if prod then pure "" else toString <$> IO.monoMsNow
      IO.FS.writeFile (outDir / "ssr.mjs") (ssrWrapper appName cacheKey)
  | none =>
      IO.println s!"  note: no `Qed.run <app>` found in {mod}, skipping the SSR handler (ssr.mjs)"
  if prod then
    if (← onPath "esbuild") then
      for f in ["app.mjs", "qed_rt.mjs", "qed_dom.mjs", "qed_host.mjs", "ssr_app.mjs", "qed_ssr_host.mjs"] do
        let p := outDir / f
...
/-- Serve a bundle: the node server when the build has an SSR handler (and node is
    present), else a plain static file server. -/
def serveDir (dir : FilePath) (dev : Bool := false) : IO UInt32 := do
  if (← (dir / "ssr.mjs").pathExists) && (← onPath "node") then
    let args := #[(← runtimeFile "qed_serve.mjs").toString, dir.toString, devPort]
    sh "node" (if dev then args.push "--dev" else args)
  else
...
def cmdTest : IO UInt32 := do
  ...
  if (← (FilePath.mk "test" / "cli_scaffold_test.mjs").pathExists) then
    step "checking the new-app quickstart"
    if (← sh "node" #["test/cli_scaffold_test.mjs"]) != 0 then failed := true
  ...
  -- Bookshelf SSR: the generated request handler (ssr.mjs) renders each route server-side.
...
def cmdDev : IO UInt32 := do
  ...
  writeBuildId devDir
  let ssr := (← (devDir / "ssr.mjs").pathExists) && (← onPath "node")
  let serveJs ← runtimeFile "qed_serve.mjs"
  let _server ← IO.Process.spawn <|
    if ssr then
      { cmd := "node",
        args := #[serveJs.toString, devDir.toString, devPort, "--dev"],
        stdin := .null, stdout := .null, stderr := .inherit }
    else
      { cmd := "python3", args := #["-m", "http.server", devPort, "--directory", devDir.toString],
        stdin := .null, stdout := .null, stderr := .inherit }
  let ssrTag := if ssr then "   (SSR)" else ""
  IO.println (green s!"\nqed dev → http://localhost:{devPort}{ssrTag}   (watching; Ctrl-C to stop)\n")
  watchLoop marker
  return 0
...
def cmdNew (dir : String) : IO UInt32 := do
  ...
  IO.FS.writeFile (root / "lean-toolchain")
    (← IO.FS.readFile ((← frameworkHome) / "lean-toolchain"))
  ...
  IO.FS.writeFile (root / "Web.lean") <|
    "import App\nimport Qed.Driver\n\ndef main : IO Unit := Qed.run Quantity.app\n"
...
def usage : IO UInt32 := do
  ...
  IO.println "  start|preview  serve the build (server-rendered when ssr.mjs is present)"
